...
const city= async(req,res)=>{
    try {
        const data=new Cities({
            ...
            placeImgUrl:req.body.placeImgUrl
        })
...
const states = async(req,res)=>{
    try{
    const data=capitalizeFirstLetter(req.params.state_name)
    const page=req.query.page
    const limit=req.query.limit
    const state = await Cities.find({$or:[
        {state_name:data},
        {city_name:data},
        {tourist_place:data}
    ]})
...
const stateCity= async(req,res)=>{
    try {
        ...
        const city= await Cities.find({$or:[
...
const search= async(req,res)=>{
    try{
        ...
        const placeData=await Cities.find({$or:[
        ...
        if(placeData.length<0){
            ...
            const weatherPromises = placeData.map(async (place) => {
                const placeWeather = await fetchWeatherData(place.city_name);
                return { ...place.toObject(), weather: placeWeather };
              });
              const placeDataWithWeather = await Promise.all(weatherPromises);
            res.status(200).send({placeDataWithWeather})
        }
    }catch(error){
        res.status(400).send({success:false,message:error.message})
    }
